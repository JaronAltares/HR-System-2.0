...
export default function LoginPage({ onEmailLogin, onGoogleLogin, authError }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // 🔄 FIXED: Remove 'if (authError)' structural trap. Always toggle loading state 
      // off when the asynchronous request finishes to unfreeze UI elements cleanly.
      setIsLoading(false);
    }
  }
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            <div className="space-y-1.5">
              ...
              <input
                ...
                className={`
                  ...
                  ${errors.email ? "border-red-400 focus:ring-2 focus:ring-red-200" : "border-gray-300 focus:ring-2"}
                `}
                style={!errors.email ? { "--tw-ring-color": "#59ABBD33" } : {}}
                onFocus={(e) => {
                  if (!errors.email) {
              ...
              <div className="relative">
                <input
                  ...
                  className={`
                    ...
                    transition-all duration-200 outline-none
                    disabled:bg-gray-100 disabled:cursor-not-allowed
                    ${errors.password ? "border-red-400 focus:ring-2 focus:ring-red-200" : "border-gray-300"}
                  `}
                  onFocus={(e) => {
                    if (!errors.password) {
                      e.target.style.borderColor = "#59ABBD";
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-semibold text-white transition-all duration-200 shadow-sm disabled:cursor-not-allowed disabled:opacity-70 hover:opacity-90 active:scale-[0.99]"
              style={{ backgroundColor: "#1B263B" }}
              onMouseEnter={(e) => { if (!isLoading) e.target.style.backgroundColor = "#59ABBD"; }}
              onMouseLeave={(e) => { if (!isLoading) e.target.style.backgroundColor = "#1B263B"; }}
            >
              {isLoading ? (
                <>
