...
function AddJobHistoryModal({ empNo, onClose, onSave }) {
  const EMPTY = { empNo, jobCode: "", deptCode: "", effDate: "", salary: "" };
  ...
  return (
    <Modal title="Add Job History" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input id="empNo"   label="Emp No."   value={form.empNo} disabled />
          <Input id="effDate" label="Effective Date" type="date"
            value={form.effDate} onChange={set("effDate")}
            required error={errors.effDate} />
        </div>
        <div className="grid grid-cols-2 gap-4">
        ...
        <Input id="salary" label="Salary (optional)" type="number"
  ...
  return (
    <Modal title={`Edit Job History — ${row.empNo}`} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input id="e-jobCode"  label="Job Code"  value={form.jobCode}  disabled />
          <Input id="e-effDate"  label="Eff. Date" value={fmt(form.effDate)} disabled />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input id="e-deptCode" label="Dept Code" value={form.deptCode}
            onChange={set("deptCode")} />
          <Input id="e-salary"   label="Salary"    type="number"
            value={form.salary} onChange={set("salary")} />
        </div>
        <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
          <button onClick={onClose} disabled={isLoading}
        ...
        <div className="flex gap-3 p-4 rounded-xl bg-red-50 border border-red-200">
          ...
          <div>
            <p className="text-sm font-semibold text-red-700">
              This will soft-delete this job history row.
            </p>
...
export default function JobHistory() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      if (empNo) {
        const { data: emps } = await employeeService.getEmployees(
          currentUser?.user_type ?? "USER"
        );
        const found = (emps ?? []).find(e => e.empno === empNo);
    ...
    <div className="space-y-5">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Emp No: <span className="font-mono font-semibold"
                    style={{ color: "#59ABBD" }}>{employee.empno}</span>
                  {" · "}
                  {employee.gender === "M" ? "Male" : "Female"}
                  {" · "}
                  Hired: {fmt(employee.hiredate)}
                </p>
              </div>
            </div>
            {isPrivileged && (
      ...
      {empErr && (
        <p className="text-sm text-amber-600 bg-amber-50 rounded-lg px-4 py-3
                      border border-amber-200">{empErr}</p>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ backgroundColor: "#1B263B" }}>
                {[
                  "Emp No.", "Job Code", "Job Title", "Dept Code",
                  "Department", "Eff. Date",
