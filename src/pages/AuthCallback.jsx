...
// FIX: Was querying employee table (hardcoded empno="00001") to decide
// whether the OAuth login succeeds. This let any Google login through as long
// as employee 00001 exists, ignoring record_status entirely.
// Now correctly checks the `user` table for record_status = 'ACTIVE'.
...
export default function AuthCallback() {
  ...
  useEffect(() => {
    ...
    async function checkUserStatus(session) {
      ...
      if (error || !userRow) {
        console.error("AuthCallback: user row not found", error);
        return false;
      }

      return userRow.record_status === "ACTIVE";
    }
    ...
    async function handleCallback() {
      const {
        ...
        if (event === "SIGNED_IN" && session) {
          const isActive = await checkUserStatus(session);
      ...
      // Fallback: handle already-cached session
      const {
        data: { session },
