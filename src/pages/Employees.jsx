...
function Modal({ title, onClose, children, width = "max-w-lg" }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className={`w-full ${width} bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh]`}>
    ...
    <Modal title="Add New Employee" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input id="empno" label="Employee No." value={form.empno} onChange={set("empno")} maxLength={5} required error={errors.empno} />
          <Select id="gender" label="Gender" value={form.gender} onChange={set("gender")} required>
            <option value="M">Male</option>
            <option value="F">Female</option>
          </Select>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input id="lastname"  label="Last Name"  value={form.lastname} onChange={set("lastname")}  maxLength={15} required error={errors.lastname} />
          <Input id="firstname" label="First Name" value={form.firstname} onChange={set("firstname")} maxLength={15} required error={errors.firstname} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input id="birthdate" label="Birth Date" type="date" value={form.birthdate} onChange={set("birthdate")} required error={errors.birthdate} />
          <Input id="hiredate" label="Hire Date" type="date" value={form.hiredate} onChange={set("hiredate")} required error={errors.hiredate} />
        </div>
        <div className="grid grid-cols-1">
          <Input id="sepDate" label="Separation Date (optional)" type="date" value={form.sepDate} onChange={set("sepDate")} />
        </div>
        <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
          <button onClick={onClose} disabled={isLoading} className="px-4 py-2.5 rounded-lg text-sm font-medium text-gray-600 border border-gray-300 hover:bg-gray-50 transition-colors">Cancel</button>
          <PrimaryBtn onClick={handleSave} loading={isLoading}>Save Employee</PrimaryBtn>
        </div>
      </div>
    </Modal>
  );
  ...
  return (
    <Modal title={`Edit Employee — ${employee.empno}`} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input id="e-lastname"  label="Last Name"  value={form.lastname} onChange={set("lastname")}  maxLength={15} />
          <Input id="e-firstname" label="First Name" value={form.firstname} onChange={set("firstname")} maxLength={15} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Input id="e-birthdate" label="Birth Date" type="date" value={form.birthdate} onChange={set("birthdate")} />
          <Input id="e-hiredate"  label="Hire Date"  type="date" value={form.hiredate}  onChange={set("hiredate")} />
        </div>
        <div className="grid grid-cols-1">
          <Input id="e-sepDate" label="Separation Date (optional)" type="date" value={form.sepDate} onChange={set("sepDate")} />
        </div>
        <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
          <button onClick={onClose} disabled={isLoading} className="px-4 py-2.5 rounded-lg text-sm font-medium text-gray-600 border border-gray-300 hover:bg-gray-50 transition-colors">Cancel</button>
    ...
    <Modal title="Confirm Soft Delete" onClose={onClose} width="max-w-md">
      <div className="space-y-4">
        <div className="flex gap-3 p-4 rounded-xl bg-red-50 border border-red-200">
          ...
          <div>
            <p className="text-sm font-semibold text-red-700">This will soft-delete this employee.</p>
          </div>
        </div>
        <div className="rounded-xl bg-gray-50 border border-gray-200 px-4 py-3">
          <p className="text-sm font-semibold" style={{ color: "#1B263B" }}>{employee.lastname}, {employee.firstname}</p>
          <p className="text-xs text-gray-500 mt-0.5">Emp No: {employee.empno}</p>
        </div>
        <div className="flex justify-end gap-3 pt-1">
          <button onClick={onClose} disabled={isLoading} className="px-4 py-2.5 rounded-lg text-sm font-medium text-gray-600 border border-gray-300 hover:bg-gray-50 transition-colors">Cancel</button>
          <button onClick={handleConfirm} disabled={isLoading} className="px-5 py-2.5 rounded-lg text-sm font-semibold text-white bg-red-600 hover:bg-red-700 transition-colors">
...
export default function Employees() {
  ...
  async function handleDelete() {
    if (!deleteRow) return;
    await employeeService.softDeleteEmployee(deleteRow.empno);
    const response = await employeeService.getEmployees(currentUser?.user_type ?? "USER");
    if (response.data) setEmployees(response.data);
  }
    ...
    return (
      <div className="flex items-center justify-center py-20 w-full h-full">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-gray-500">Loading directory listings…</p>
        </div>
      </div>
    );
  }
  ...
  if (fetchErr) {
    return (
      <div className="flex items-center justify-center py-20 border border-dashed rounded-2xl border-red-200 bg-red-50/50 max-w-xl mx-auto w-full">
        <div className="text-center space-y-4 px-6">
          <div className="text-red-500 font-semibold text-base">⚠️ Diagnostic Error Triggered</div>
          <p className="text-gray-600 text-sm font-mono bg-white p-3 rounded-xl border border-red-100 max-h-40 overflow-y-auto">{fetchErr}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5 w-full">
      ...
      <div className="relative max-w-sm">
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, empno…" className="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm bg-white outline-none" />
      </div>

      <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm bg-white w-full">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {visible.length === 0 ? (
                ...
                <tr key={emp.empno} className="transition-colors duration-100" style={{ backgroundColor: i % 2 === 0 ? "#fff" : "#F9FAFB" }}>
                  <td className="px-4 py-3 font-mono font-semibold whitespace-nowrap" style={{ color: "#59ABBD" }}>{emp.empno}</td>
                  <td className="px-4 py-3 font-medium whitespace-nowrap" style={{ color: "#1B263B" }}>{emp.lastname || emp.Lastname}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-gray-700">{emp.firstname}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-gray-600">{emp.gender === "M" ? "Male" : "Female"}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-gray-600">{fmt(emp.hiredate)}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-gray-600">{fmt(emp.sepDate)}</td>
      ...
      {showAdd && <AddEmployeeModal onClose={() => setShowAdd(false)} onSave={handleAdd} />}
      {editRow && <EditEmployeeModal employee={editRow} onClose={() => setEditRow(null)} onSave={handleEdit} />}
      {deleteRow && <SoftDeleteDialog employee={deleteRow} onClose={() => setDeleteRow(null)} onConfirm={handleDelete} />}
    </div>
  );
}